...
tol = 1e-10                         # Convergence threshold for Newton-Raphson
...
for i, t in enumerate(t_grid):
    ...
    LHS_mat_small = np.block([[Φ_r_mod.T, np.zeros((3, 1))],
                              [Φ_p_mod.T, pendulum.p]])
    RHS_mat_small = np.block([[Fg - M @ ddq_k[0:3]], [τ - Jp @ ddq_k[3:7]]])

    x_small = np.linalg.solve(LHS_mat_small, RHS_mat_small)
    ...
    assert np.amax(np.abs(x[8:] - x_small[0:6])) < tol, "small/large mismatch"
    # assert np.abs(x[7] - x_small[6]) < tol, "small/large mismatch"

    mult = (-0.5 * G @ Φ_p_mod.T).T
    for j, mult_i in enumerate(mult):
        nr[i, :, j] = mult_i.T * λ[j]
